package com.example.AuthorizationAndAuthentication.configurations;

import com.example.AuthorizationAndAuthentication.exceptionHandling.CustomAccessDeniedHandler;
import com.example.AuthorizationAndAuthentication.exceptionHandling.CustomBasicEntryPoint;
import com.example.AuthorizationAndAuthentication.filter.CustomCsrfCookieFilter;
import com.example.AuthorizationAndAuthentication.filter.RequestValidationBeforeFilter;
        ...
        httpSecurity
               ...
               // .requiresChannel(httpSecurityRequiresChannelConfigurer -> httpSecurityRequiresChannelConfigurer.anyRequest().requiresSecure()) // For HTTPS
                ...
                .authorizeHttpRequests((request) -> request
                        //.access method we can use expression to check for role or authorization
               /* .requestMatchers("/account").hasAuthority("VIEWACCOUNT")// these are authenticaed
                .requestMatchers("/balance").hasAuthority("VIEWBALANCE")// these are authenticaed
                .requestMatchers("/cards").hasAuthority("VIEWCARDS")// these are authenticaed
                .requestMatchers("/loans").hasAuthority("VIEWLOANS")// these are authenticaed
                .requestMatchers("/notices").hasAnyAuthority("VIEWLOANS", "VIEWACCOUNT")// any of these*/
                 // We Don't need ROLE_ prefix over here. Just in DB
                .requestMatchers("/account").hasRole("ADMIN")// these are authenticaed
                .requestMatchers("/balance").hasRole("ADMIN")// these are authenticaed
                .requestMatchers("/cards").hasRole("USER")// these are authenticaed
                .requestMatchers("/loans").hasRole("USER")// these are authenticaed
                .requestMatchers("/notices").hasAnyRole("ADMIN", "USER")// any of the
                .requestMatchers("/user").authenticated()// Any authenticated user

                .requestMatchers("/contacts", "/error", "/invalidSession", "/register").permitAll());// All can access
